  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        router.push(data.user.role === "admin" ? "/admin/dashboard" : "/reporter/dashboard")
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900 mb-2">AI Procurement</h1>
        ...
        <Card className="p-8 shadow-lg border-0">
          ...
          <div className="mt-8 pt-8 border-t border-slate-200">
            <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-4">Demo Credentials</p>
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <p className="text-slate-500">reporter@example.com / reporter123</p>
              </div>
            </div>
          </div>
        </Card>

        <p className="text-center text-slate-600 text-sm mt-6">Protected application • Use demo credentials</p>
      </div>
    </div>
  )
}
