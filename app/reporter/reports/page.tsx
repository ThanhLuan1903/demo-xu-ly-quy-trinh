...
import { Plus, Image as ImgIcon, Video, RefreshCw, Send, MessageSquare, X } from "lucide-react"
...
export default function ReportsPage() {
  ...
  const fetchIncidents = async (keepSelection = true) => {
    try {
      ...
      const res = await fetch(`/api/incidents?viewer_id=${me.id}&role=reporter`, { cache: "no-store" })
      ...
      <div className="p-4 md:p-8 bg-slate-50 min-h-screen">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              <h1 className="text-3xl font-bold text-slate-900 mb-2">Báo cáo sự cố của tôi</h1>
              <p className="text-slate-600">Chỉ hiển thị các báo cáo do bạn tạo.</p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button onClick={() => router.push("/reporter/reports/new")} className="bg-blue-600 hover:bg-blue-700">
          ...
          {loading ? (
            ...
            <Card className="p-6 border-0">
              <p className="text-slate-600">Hiện tại bạn không có báo cáo nào.</p>
            </Card>
          ) : (
              ...
              {incidents.map((incident) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3 flex-wrap">
                      <div className="flex gap-2 items-center flex-wrap">
                        ...
                        {incident.assignee?.name ? (
                          <span className="text-xs text-slate-500">• Người xử lý: {incident.assignee.name}</span>
                        ) : null}
                      </div>
                    ...
                    <div className="mt-2 text-xs text-slate-400">Nhấn để xem chi tiết</div>
                  </Card>
                )
              })}
            </div>
          ...
          <Dialog open={!!selectedIncident} onOpenChange={(open) => !open && closeDetail()}>
                ...
                <div className="flex flex-col max-h-[85vh]">
                  ...
                  <div className="px-6 py-4 border-b bg-white">
                    <DialogHeader>
                      <DialogTitle className="flex items-start justify-between gap-3">
                        <div>
                          <div className="text-xl font-bold text-slate-900">Chi tiết sự cố</div>
                          <DialogDescription className="text-xs text-slate-500 mt-1">
                            Mã: <span className="font-mono">{selectedIncident.id}</span>
                          </DialogDescription>
                        </div>
                  ...
                  <div className="px-6 py-5 space-y-4 overflow-y-auto bg-slate-50">
                    ...
                    {selectedIncident.proposed_fix ? (
                      <div>
                        <span className="text-sm font-semibold text-slate-900">Phương án khắc phục: </span>
                        <span className="text-sm text-slate-700 whitespace-pre-wrap">{selectedIncident.proposed_fix}</span>
                      </div>
                    ) : null}
                  ...
                  <div className="px-6 py-4 border-t bg-white flex justify-end">
                    <Button variant="outline" className="bg-transparent" onClick={closeDetail} disabled={busy}>
                      Đóng
                    </Button>
                  </div>
                </div>
