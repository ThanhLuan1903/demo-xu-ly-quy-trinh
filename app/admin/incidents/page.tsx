...
export default function IncidentsPage() {
  ...
  const me = useMemo(() => {
    ...
    if (!raw) return null
    try {
      return JSON.parse(raw) as { id: string; role: string; facility_id: string; name: string }
    } catch {
      return null
    }
  }, [])
  ...
  useEffect(() => {
    const t = setTimeout(() => commentsEndRef.current?.scrollIntoView({ behavior: "smooth" }), 50)
  ...
  const fetchIncidents = async (keepSelection = true) => {
    try {
      if (!me?.id) return
      setLoading(true)
      const res = await fetch(`/api/incidents?viewer_id=${me.id}&role=admin`, { cache: "no-store" })
      const data = await res.json()
      const list = (data || []) as Incident[]
      setIncidents(list)

      if (keepSelection && selectedIncident?.id) {
        const refreshed = list.find((x) => String(x.id) === String(selectedIncident.id)) || null
  ...
  const selectIncident = (incidentId: string) => {
    ...
    if (selectedIncident?.id && String(selectedIncident.id) === String(incidentId)) {
      setSelectedIncident(null)
      return
    }

    // chọn từ list hiện tại để có đủ fields
    const found = incidents.find((x) => String(x.id) === String(incidentId)) || null
  ...
  const disableApprove = busy || !selectedIncident || selectedStatus === "resolved" || statusDisable(selectedStatus as any)
  const disableReject = busy || !selectedIncident || selectedStatus === "rejected" || statusDisable(selectedStatus as any)
      ...
      <div className="p-4 md:p-8 bg-slate-50 min-h-screen">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              <h1 className="text-3xl font-bold text-slate-900 mb-2">Sự cố được giao cho tôi</h1>
              <p className="text-slate-600">
                Chỉ hiển thị các báo cáo có <span className="font-semibold">assigned_to</span> = admin đang đăng nhập.
              </p>
            </div>

            <div className="flex items-center gap-2">
              {selectedIncident ? (
                <Button variant="outline" onClick={closeDetail} disabled={busy} className="bg-transparent">
            ...
            <div className="lg:col-span-2">
              {loading ? (
                ...
                <div className="space-y-4">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2 gap-3">
                          <h3 className="font-semibold text-slate-900 flex-1">{incident.title}</h3>
                        ...
                        <div className="mt-3 flex items-center justify-between gap-3 flex-wrap">
                          <div className="flex gap-2 items-center flex-wrap">
                            ...
                            {incident.reporter?.name ? (
                              <span className="text-xs text-slate-500">• Reporter: {incident.reporter.name}</span>
                            ) : null}
                          </div>
                        ...
                        <div className="mt-2 text-xs text-slate-400">
                          {active ? "Nhấn lại để đóng chi tiết" : "Nhấn để mở chi tiết"}
                        </div>
                      </Card>
                    )
                  })}
                  ...
                  {incidents.length === 0 && (
                    <Card className="p-6 border-0 text-center text-slate-500">
                      <p>Hiện chưa có sự cố nào được giao cho bạn.</p>
                    </Card>
                  )}
                </div>
              )}
                ...
                <Card className="p-6 border-0 sticky top-8">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="outline" size="icon" className="bg-transparent" onClick={closeDetail} disabled={busy}>
                        <X className="w-4 h-4" />
                      </Button>
                    </div>
                  </div>
                  ...
                  <p className="text-sm text-slate-600 mt-2 whitespace-pre-wrap">{selectedIncident.description}</p>

                  {selectedIncident.proposed_fix ? (
                    <div className="mt-4 bg-slate-50 border rounded-lg p-3">
                      <p className="text-xs font-semibold text-slate-900 mb-1">Đề xuất cách khắc phục</p>
                      <p className="text-sm text-slate-700 whitespace-pre-wrap">{selectedIncident.proposed_fix}</p>
                    </div>
                  ) : null}

                  <div className="mt-4 text-xs text-slate-500">
                    Tạo: {new Date(selectedIncident.created_at).toLocaleString("vi-VN")} <br />
                    Cập nhật: {new Date(selectedIncident.updated_at).toLocaleString("vi-VN")}
                  </div>
                  ...
                  <div className="mt-5">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => updateIncidentStatus(selectedIncident.id, value as any)}
                      disabled={busy}
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  {(selectedIncident.attachments || []).length > 0 && (
                    <div className="mt-5">
                      <p className="text-sm font-semibold text-slate-900 mb-2">Tệp đính kèm</p>
                        ...
                        {(selectedIncident.attachments || [])
                          ...
                          .map((a) => (
                            <button
                              ...
                              className="h-20 w-full rounded-md overflow-hidden border bg-white"
                              title="Nhấn để xem"
                            >
                              {a.type === "image" ? (
                                // eslint-disable-next-line @next/next/no-img-element
                                <img src={a.url} alt={a.filename || "att"} className="w-full h-full object-cover" />
                              ) : (
                  ...
                  <div className="mt-6 pt-4 border-t">
                    ...
                    <div className="space-y-2 mb-3 max-h-56 overflow-y-auto pr-1">
                      {(selectedIncident.comments || []).length === 0 ? (
                        <p className="text-xs text-slate-500">Chưa có bình luận.</p>
                      ) : (
                        (selectedIncident.comments || []).map((c) => {
                          const isMe = !!me?.id && String(c.author_id) === String(me.id)
                          return (
                            <div key={c.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                              ...
                              >
                                <p className={`text-[11px] mb-1 ${isMe ? "text-blue-50" : "text-slate-500"}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <Button size="sm" onClick={addComment} disabled={busy} className="bg-blue-600 hover:bg-blue-700">
                  ...
                  <div className="mt-4 pt-4 border-t flex gap-2">
                    ...
                    >
                      ...
                      {selectedIncident.status === "resolved" ? "Đã xác nhận" : "Xác nhận"}
                    </Button>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (!selectedIncident) return
                        if (selectedIncident.status === "rejected") return
                        updateIncidentStatus(selectedIncident.id, "rejected")
                      }}
                    >
                      <X className="w-4 h-4 mr-1" />
                      {selectedIncident.status === "rejected" ? "Đã từ chối" : "Từ chối"}
                    </Button>
                  </div>
                </Card>
              ) : (
